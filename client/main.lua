...
RegisterNetEvent('esx_holdupfleeca:currentlyRobbing')
AddEventHandler('esx_holdupfleeca:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_holdupfleeca:killBlip')
AddEventHandler('esx_holdupfleeca:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_holdupfleeca:setBlip')
AddEventHandler('esx_holdupfleeca:setBlip', function(position)
...
RegisterNetEvent('esx_holdupfleeca:tooFar')
AddEventHandler('esx_holdupfleeca:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_holdupfleeca:robberyComplete')
AddEventHandler('esx_holdupfleeca:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_holdupfleeca:startTimer')
AddEventHandler('esx_holdupfleeca:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_holdupfleeca:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_holdupfleeca:tooFar', store)
			end
		end
	end
end)
